...
function analyze2013()
    ...
    for f = funcs
        % 读取目标函数对应的分组结果
        filename = sprintf('./groupResult/cec2013/F%02d.mat', f);
        % 加载对应的分组结果的信息，seps，nonseps，FEs
        load(filename);
        s=[];
        m=0;
...
%         fprintf('FEs used: %d\n', FEs);
        ...
        for i=[1:1:length(nonseps)]
            ...
            if(flag)
                ldim = 1;
                for g=1:length(s)
                    % 从所有的变量中找和当前未分组变量重合的部分的长度
                    index = ldim:ldim+s(g)-1;
    ...
    xlswrite('results_2013.xlsx',nonsep_group_acc,'D1:D15');
