...
function gval = decc(fname, func_num, dim, Lbound, Ubound, popsize, Max_FEs, runindex, problem)
    ...
    % the grouping results acquired by FCA-G
    filename = sprintf('./FCA_G/groupResult/cec%d/F%02d.mat',problem,func_num);
    load(filename);
    group_num = size(group, 2);
    
	% The rest is the same as DECC
    display = 1;
    frequency = 100;
